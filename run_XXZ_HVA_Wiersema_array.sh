...
#SBATCH -J XXZ                             # Job name: XXZ
#SBATCH --qos=normal                       # Set quality of service (queue priority level)
#SBATCH -o %j_XXZ.out                      # Output file, %j = job ID (%A = main job ID, %a = array index)
...
#SBATCH --array=6                        # Job array index, here only task 3 is submitted
...
num_iter_oicd=10 # $(( layer * 10 * 100 ))      # e.g., 400 * 100 = 40000
num_iter_rcd=10 # ${num_iter_oicd}             # Same as OICD
num_iter_gd=$(( (1 * num_iter_oicd) / (2 * layer * 4) + 1 ))  # Adjusted GD iterations

JOB_ID="${SLURM_JOB_ID}"                  # Store SLURM job ID in a variable

# Run the Python script with arguments
python run_XXZ_HVA_Wiersema_new.py \
  --num_q ${num_q} \
  --layer ${layer} \
  --lr_gd ${lr_gd} \
  --lr_rcd ${lr_rcd} \
