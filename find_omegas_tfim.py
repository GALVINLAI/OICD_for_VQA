...
def extract_and_save_compact_omegas(
    ...
    def expectation_loss(weights):
        return expectation_loss_grad(
            num_q, layer, weights, circuit=circuit_HVA_TIFM, obs=tfim_op
        )
...
if __name__ == "__main__":
    ...
    output_txt_path = os.path.join("find_omegas_tfim_results", "all_tables_summary.txt")
    os.makedirs("find_omegas_tfim_results", exist_ok=True)

    verbose_to_console = True  # ✅ 控制是否输出到屏幕

    with open(output_txt_path, "w", encoding="utf-8") as f:

        for num_q in range(4, 18, 2):
            layer = 2 * num_q
            weights_dict = extract_and_save_compact_omegas(
                num_q=num_q,
                layer=layer,
                verbose=False,
                save_dir="find_omegas_tfim_results"
            )
...
# with open("find_omegas_tfim_results/compact_omegas_q5_l2.pkl", "rb") as f:
