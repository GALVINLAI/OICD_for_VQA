...
def oicd_var(estimate_loss_fun,
        expectation_loss,
        variance, #####################
        # fidelity, 
        ground_energy,
    ...
    for i in t:
        ...
        # MSE with constant variance
        def mse_const(interp_points):
        ...
        # MSE with nonconstant variance
        def mse_var(interp_points):
            A = interp_matrix(interp_points, omegas)
            d = np.array([1/var_fun(x) for x in interp_points])
            SIGMA_inv = np.diag(d)
            regularized_matrix = A.T @ SIGMA_inv @ A + 1e-6 * np.eye(A.shape[1]) 
            return np.trace(np.linalg.inv(regularized_matrix))

        def optimal_interp_points(mse): 
            r = len(omegas)
            # bounds = [(-1e6, 1e6) for _ in range(2 * r + 1)]  # This is a loose boundary range
            bounds = [(0, 4*np.pi) for _ in range(2 * r + 1)]
            result_mse = differential_evolution(mse, bounds, strategy='best1bin', maxiter=500)
        ...
        opt_mse_var, opt_mse_var_interp_points, Ainv_var = optimal_interp_points(mse_var)
        opt_mse_const, opt_mse_const_interp_points, Ainv_const = optimal_interp_points(mse_const)

        print('iteration', i)
        print('opt_mse_var', opt_mse_var)
        print('opt_mse_const', opt_mse_const)
        print('mse_const for optimal nodes of mse_var', mse_const(opt_mse_var_interp_points))
        print('mse_var for optimal nodes of mse_const', mse_var(opt_mse_const_interp_points))
        mse1.append(opt_mse_var)
        mse2.append(opt_mse_const)       
        mse3.append(mse_const(opt_mse_var_interp_points))
        mse4.append(mse_var(opt_mse_const_interp_points))
        print()

        # print('opt_interp_points', opt_mse_var_interp_points)
        # print('Ainv', Ainv_var)

        if mse == 'nonconstant':
            interp_nodes = opt_mse_var_interp_points
            inv_A = Ainv_var
        else:
            interp_nodes = opt_mse_const_interp_points
            inv_A = Ainv_const


        # if mse == 'nonconstant':
        #     opt_mse, opt_interp_points, Ainv = optimal_interp_points(mse_var)

        #     print('iteration', i)
        #     print('mse_var', opt_mse)
        #     print('opt_interp_points', opt_interp_points)
        #     print('Ainv', Ainv)

        #     print('mse_const for optimal nodes of mse_var', mse_const(opt_interp_points))
        #     mse_const_list.append(mse_const(opt_interp_points))
        #     mse_var_list.append(opt_mse)

        #     print()
        # else:
        #     opt_mse, opt_interp_points, Ainv = optimal_interp_points(mse_const)    

        #     print('iteration', i)
        #     print('mse_const', opt_mse)
        #     print('opt_interp_points', opt_interp_points)
        #     print('Ainv', Ainv)

        #     print('mse_var for optimal nodes of mse_const', mse_var(opt_interp_points))
        #     mse_const_list.append(opt_mse)
        #     mse_var_list.append(mse_var(opt_interp_points))
    ...
    return [mse1,mse2,mse3,mse4], weights, success, best_expected_record_value, func_count_record_value, expected_record_value, metric_record
